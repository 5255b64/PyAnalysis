...
使用PCA降维的 凝聚层次聚类削减算法
"""
def run(attributes: list, cluster_num: int, pca_n_components:int = PCA_N_COMPONENTS):
    """

    :param attributes_pca:  输入特征
    :param cluster_num: 分类的类簇数量
    :return:    list 返回筛选后剩下的测试用例的编号（返回值数量与cluster_num相同）
    """
    # 引入PCA降维方法 对attributes进行降维
    attributes_pca = pca.run(attributes, n_components=pca_n_components)
